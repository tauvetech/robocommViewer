...
//       // serialPort->clear();
// //       serialPort->write("readAccelMagGyro\n");

//}

void Dialog::readData()
{
    //    qDebug()<<"on slot readData begin!";
    //    QByteArray data = serialPort->readAll();
    //    qDebug()<< "data receive:" << data;
    //    qDebug()<< "data lenght:" << data.length();
    //    qDebug()<<"on slot readData end!";
    //   // serialPort->write("readAccel\n");
    ...
    QByteArray dataEntete = serialPort->readLine();
    qDebug()<< dataEntete;
    ...
    if (dataEntete[0] == 'A')
    {
        ...
        QString cptStr;
        cptStr.setNum(cpt);
        ui->qwtPlotAccel->setAxisTitle(QwtPlot::xBottom, "X (nb samples)"+cptStr);
        cpt++;
        QByteArray data = serialPort->readLine();
        unsigned long received_value =0;
        ...
        if (cpt>100)
        {
            ui->qwtPlotAccel->setAxisScale( QwtPlot::xBottom, cpt-100, cpt );
        }

        pointF_Accel_X.setX(cpt);
        pointF_Accel_X.setY(accel_x_axis);
        points_Accel_X.append(pointF_Accel_X);
        curveAccel_X->setSamples( points_Accel_X );

        pointF_Accel_Y.setX(cpt);
        pointF_Accel_Y.setY(accel_y_axis);
        points_Accel_Y.append(pointF_Accel_Y);
        curveAccel_Y->setSamples( points_Accel_Y );

        pointF_Accel_Z.setX(cpt);
        pointF_Accel_Z.setY(accel_z_axis);
        points_Accel_Z.append(pointF_Accel_Z);
        curveAccel_Z->setSamples( points_Accel_Z );
    ...
    if (dataEntete[0] == 'M')
    {
        ...
        static unsigned long cptMag=0;
        QString cptMagStr;
        cptMagStr.setNum(cptMag);
        ui->qwtPlotMag->setAxisTitle(QwtPlot::xBottom, "(nb samples: "+cptMagStr+" )");
        cptMag++;
        ...
        {
            if(dataMagZ.at(i) >= '0' && dataMagZ.at(i) <= '9')
                received_valueMagZ = received_valueMagZ*16+(dataMagZ.at(i)-'0');
            else if(dataMagZ.at(i) >= 'a' && dataMagZ.at(i) <= 'f')
                received_valueMagZ = received_valueMagZ*16+(dataMagZ.at(i)-'a'+10);
            else if(dataMagZ.at(i) >= 'A' && dataMagZ.at(i) <= 'F')
                received_valueMagZ = received_valueMagZ*16+(dataMagZ.at(i)-'A'+10);
        ...
        if (cptMag>100)
        {
            ui->qwtPlotMag->setAxisScale( QwtPlot::xBottom, cptMag-100, cptMag );
        }

        pointF_Mag_X.setX(cptMag);
        pointF_Mag_X.setY(mag_x_axis);
        points_Mag_X.append(pointF_Mag_X);
        curve_Mag_X->setSamples( points_Mag_X );

        pointF_Mag_Y.setX(cptMag);
        pointF_Mag_Y.setY(mag_y_axis);
        points_Mag_Y.append(pointF_Mag_Y);
        curve_Mag_Y->setSamples( points_Mag_Y );

        pointF_Mag_Z.setX(cptMag);
        pointF_Mag_Z.setY(mag_z_axis);
        points_Mag_Z.append(pointF_Mag_Z);
        curve_Mag_Z->setSamples( points_Mag_Z );
        ///////////////////////////////////////////////////////////////////////////////////////
    ...
    if (dataEntete[0] == 'G')
    {
        ...
        static unsigned long cptGyro=0;
        QString cptGyroStr;
        cptGyroStr.setNum(cptGyro);
        ui->qwtPlotGyro->setAxisTitle(QwtPlot::xBottom, "(nb samples: "+cptGyroStr+" )");
        cptGyro++;
        ...
        if (cptGyro>100)
        {
            ui->qwtPlotGyro->setAxisScale( QwtPlot::xBottom, cptGyro-100, cptGyro );
        }

        pointF_Gyro_X.setX(cptGyro);
        pointF_Gyro_X.setY(gyro_x_axis);
        points_Gyro_X.append(pointF_Gyro_X);
        curve_Gyro_X->setSamples( points_Gyro_X );

        pointF_Gyro_Y.setX(cptGyro);
        pointF_Gyro_Y.setY(gyro_y_axis);
        points_Gyro_Y.append(pointF_Gyro_Y);
        curve_Gyro_Y->setSamples( points_Gyro_Y );

        pointF_Gyro_Z.setX(cptGyro);
        pointF_Gyro_Z.setY(gyro_z_axis);
        points_Gyro_Z.append(pointF_Gyro_Z);
        curve_Gyro_Z->setSamples( points_Gyro_Z );
        /////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Dialog::on_pbReadAccel_clicked()
{
    serialPort->write("readAccel\n");
}

void Dialog::on_pbReadMag_clicked()
{
    serialPort->write("readMag\n");
}

void Dialog::on_pbReadGyro_clicked()
{
    serialPort->write("readGyro\n");
}
